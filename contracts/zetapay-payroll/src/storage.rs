...
impl Storage {
    ...
    pub fn mark_proof_processed(env: &Env, employer: &Address, proof_hash: &BytesN<32>) {
        env.storage()
            .persistent()
            .set(&DataKey::ProcessedProof(employer.clone(), proof_hash.clone()), &true);
    }
    ...
    pub fn set_payroll_run_summary(
        ...
        env.storage()
            .persistent()
            .set(&DataKey::PayrollRun(employer.clone(), payroll_run_hash.clone()), summary);
    }
    ...
    pub fn get_payroll_run_summary(
        ...
        env.storage()
            .persistent()
            .get(&DataKey::PayrollRun(employer.clone(), payroll_run_hash.clone()))
    }
}